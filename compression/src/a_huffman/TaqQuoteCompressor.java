...
public class TaqQuoteCompressor
{
			...
			while(len > 0){
				...
				for(int j = 0;j<used_streams.length;j++){
					...
					if (j==0){
						...
						if (writtenFirstDate){
							used_streams[0].writeRecord(intToByteArray(de.getNextDate(),4),0,4);
						}else{
	...
	private static void extract(String outdir,String path,String extractedFile) throws Exception{
		...
		try{
			...
			//now iterate
			int len = used_streams[0].readRecord(buffer, 0, 4); //read the first record from the first file
			...
			while(len > 0){
				...
				int bytesReadIn = len;
				for(int j = 0;j<compressFileNames.length;j++){
					...
					if (j==0){
						int firstd = 0;
						if (!firstDateAdded){
							firstd = dd.getFirstDate();
							//date = intToByteArray(dd.getFirstDate(),9);
							firstDateAdded = true;
						} else{
							firstd = dd.getNextDate();
				...
				len = used_streams[0].readRecord(buffer, 0, 4); //start over with the next record
			}
		}finally{
			//close everything
